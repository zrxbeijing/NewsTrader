...
class EventReturnCalculator(object):
    ...
    def _calculate_return(self, start_date, end_date):
        ...
        if self.log_mode:
            price_df["stock_return"] = np.log(price_df[self.ticker]) - np.log(
                price_df["lag_stock"]
            )
            price_df["index_return"] = np.log(price_df[self.stock_index]) - np.log(
                price_df["lag_index"]
            )
        else:
            price_df["stock_return"] = (
                price_df[self.ticker] - price_df["lag_stock"]
            ) / price_df["lag_stock"]
            price_df["index_return"] = (
                price_df[self.stock_index] - price_df["lag_index"]
            ) / price_df["lag_index"]
    ...
    def _create_intermediate(self, return_df=None):
        ...
        df_template = pd.DataFrame(
            index=range(
                (
                    return_df[self.date_column].iloc[-1]
                    - return_df[self.date_column].iloc[0]
                ).days
                + 1
            ),
            columns=return_df.columns,
        )
        df_template[self.date_column] = [
            return_df[self.date_column].iloc[0] + pd.Timedelta(days=i)
            for i in range(
                (
                    return_df[self.date_column].iloc[-1]
                    - return_df[self.date_column].iloc[0]
    ...
    def _find_window_return(self, return_df=None, window_size=None):
        ...
        drift_trading = [
            index - zero_index for index, row in intermediate_df.iterrows()
        ]
        intermediate_df["drift"] = drift_trading
        # set trading_day drift as index
        window_return_df = (
            intermediate_df[intermediate_df["drift"].isin(window_list)]
            .reset_index(drop=True)[
                ["drift", self.date_column, "stock_return", "index_return"]
            ]
            .set_index("drift")
        )
    ...
    def _calculate_window_return(self, window_size=None, date_range_multiplier=7):
        ...
        start_date = self.event_date - pd.Timedelta(
            days=window_size * date_range_multiplier
        )
        end_date = self.event_date + pd.Timedelta(
            days=window_size * date_range_multiplier
        )
    ...
    def _estimate_market_model(
        self, window_distance=None, period_len=None, date_range_multiplier=2
    ):
        """
        ...
        if return_df is None or len(return_df) < period_len:
            ...
            regression_result = ols(
                formula="stock_return ~ index_return", data=return_df
            ).fit()
    ...
    def calculate_window_abnormal(
        self, window_size=None, window_distance=None, period_len=None
    ):
        """
        ...
        intercept, beta, rsquared, residual = self._estimate_market_model(
            window_distance, period_len
        )
        if window_return_df is None or rsquared is None:
            return None
        window_return_df["abnormal_return"] = (
            window_return_df["stock_return"]
            - intercept
            - beta * window_return_df["index_return"]
        )
        ...
        window_return_df["intercept"] = [
            intercept for i in range(len(window_return_df))
        ]
