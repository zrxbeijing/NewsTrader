...
def multi_event_study(df,
    ...
    if data_df:
        price_df_list = [data_df[[date_column] + [event['ticker'], event[stock_index_column]]] for event in event_list]
    else:
        price_df_list = [None for event in event_list]
    argument_list = zip(event_list, price_df_list)
    partial_func = partial(one_event_study,
    ...
    result_list = run_multitasking(func=partial_func, 
                                   argument_list=argument_list, 
                                   num_workers=worker_num,
                                   thread_or_process='process')

    # prepare final output
    date_column_list = ['date t{}'.format(str(day))
    ...
    abnormal_column_list = ['ab return t{}'.format(str(day))
                            for day in list(range(-window_size, window_size + 1))]
    series_index = date_column_list + return_column_list + abnormal_column_list + ['rsqured', 'intercept', 'beta',
                                                                                   'residual']
    final_result_list = []
    for k in range(len(result_list)):
        window_df = result_list[k]
        if window_df is not None and len(window_df) == 2 * window_size + 1:
            result = pd.Series(index=series_index,
                               data=list(window_df[date_column]) + list(window_df['stock_return']) + list(
                                   window_df['abnormal_return']) + [window_df['rsqured'][0]] + [
                                   window_df['intercept'][0]] + [window_df['beta'][0]] + [window_df['residual'][0]])
        else:
            result = pd.Series(
...
def one_event_study(event,
    ...
    :return: pd.Series
    """
    ticker, event_date, stock_index = event[ticker_column], event[date_column], event[stock_index_column]
    window_df = EventReturnCalculator(ticker=ticker,
                                      event_date=event_date,
                                      stock_index=stock_index,
                                      data_df=price_df,
                                      log_mode=log_mode,
                                      date_column=date_column).calculate_window_abnormal(window_size,
                                                                                         window_distance,
                                                                                         estimation_period)
    return window_df
