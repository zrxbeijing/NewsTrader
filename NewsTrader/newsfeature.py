...
def extract_symbols(
    ...
    outputs_list = [
        [
            ...
            if output["entity_group"] == "ORG" and output["score"] >= min_score
        ]
    ...
    for i in range(len(outputs_list)):
        ...
        if len(outputs) != 0:
            for output in outputs:
                ...
                original_word = _get_original_word(output).lower()
...
def _get_original_word(output):
    identified_word = output["title"][output["start"] : output["end"]]
    possible_prefix = ""
    for k in reversed(range(output["start"])):
        prefix = output["title"][k]
        if prefix != " ":
            possible_prefix += prefix
        else:
            break
    possible_postfix = ""
    for l in range(output["end"], len(output["title"])):
        postfix = output["title"][l]
        if postfix != " ":
            possible_postfix += postfix
        else:
            break
    original_word = possible_prefix + identified_word + possible_postfix
