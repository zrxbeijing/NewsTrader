...
def extract_symbols(titles,
                    ...
                    preferred_country: list = ['United States', 'United Kingdom', 'Canada', 'Japan', 'Germany', 'France'],
    ...
    outputs_list = [[output for output in outputs if output['entity_group'] == 'I-ORG' and output['score'] >= min_score]
    ...
    for i in range(len(outputs_list)):
        ...
        if len(outputs) != 0:
            for output in outputs:
                ...
                target_indicator = [True if original_word in name else False for name in company_names]
...
def _extract_symbol(outputs,
    ...
    for output in outputs:
        ...
        if len(sorted_index) != 0:
            ...
            query_results = [result[['long_name', 'symbol']] for result in query_results]
...
def get_stock_info(country=None, 
                   sector=None, 
                   industry=None, 
                   data_dir='.', 
    ...
    if os.path.exists(file_path):
        ...
        return stock_df
    stock_df = pd.DataFrame(fd.select_equities(country=country, sector=sector, industry=industry)).transpose()
