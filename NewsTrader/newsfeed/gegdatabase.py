...
class GegDatabase:
    ...
    """
    _MASTER_FILE_URL = "http://data.gdeltproject.org/gdeltv3/geg_gcnlapi/MASTERFILELIST.TXT"
    ...
    def _get_master_file(self):
        ...
        if not response.status_code == 0:
            print("There is something wrong with the original url for the master file! Check GDELT GEG!")
    ...
    def get_table(self):
        ...
        json_date_list = [file.split('/')[-1].split('.')[0][:8] for file in json_file_list]
        start = self.start.strftime(format="%Y%m%d")
        end = (self.end + pd.Timedelta(value=1, unit='D')).strftime(format="%Y%m%d")
        json_file_list = json_file_list[json_date_list.index(start):json_date_list.index(end)]
        ...
        result_list = run_multitasking(
            func=partial_func, argument_list=json_file_list, num_workers=worker_num, thread_or_process='process')
    ...
    def _parse_one_entry(json_file, lang='en'):
        ...
        entry_list = [entry.split(', "entities"')[0].strip() + '}' for entry in entry_list]
