...
def clean_df(df, target_column):
    ...
    # remove not text
    indicators = [True if type(row[target_column]) == str else False for index, row in df.iterrows()]
...
def drop_duplicate_news(df,
    ...
    task_list = [df[df[ticker_column] == ticker].reset_index(drop=True) for ticker in ticker_list]
    partial_func = partial(delete_duplicate_news,
                           look_back=look_back,
    ...
    result_list = run_multitasking(
        func=partial_func, argument_list=task_list, num_workers=worker_num, thread_or_process='process')

    result = pd.concat(result_list).drop(columns=[embedding_column]).reset_index(drop=True)
    return result
...
def delete_duplicate_news(sub_df, look_back: int, date_column: str, embedding_column: str, min_similarity: float = 0.8):
    """
    ...
    for index, row in sub_df.iterrows():
        ...
        date_range = [end_date - pd.Timedelta(timedelta(days=i)) for i in range(look_back)]
        # we select those news, whose dates are earlier (look_back days) and not yet been kicked out.
        indicator_list = [row_[date_column] in date_range and index_ < index and index_ not in drop_index_list
