...
class PriceData:
    ...
    def write_sqlite(self, data_df):
        ...
        if os.path.exists(os.path.join(self.data_dir, self.sqlite_file)):
            print('There is already a database! We remove it and store new data!')
            os.remove(os.path.join(self.data_dir, self.sqlite_file))
        ticker_df = data_df.drop(columns=['Date'])
        chunks = [ticker_df.iloc[:, x:x + self.chunk_size] for x in range(0, len(ticker_df.columns), self.chunk_size)]
        chunks = [data_df[['Date']].merge(chunk, left_index=True, right_index=True) for chunk in chunks]
        for i in range(len(chunks)):
            print('Writing no.{} table in total of {} tables'.format(str(i), str(len(chunks))))
            con = sqlite3.connect(os.path.join(self.data_dir, self.sqlite_file))
            chunk = chunks[i]
            chunk.to_sql('table_{}'.format(str(i)), con=con, if_exists='append', index=False)
    ...
    def read_sqlite(self):
        ...
        df_list = []
        for table in tables:
            print('Reading {} in total of {} tables'.format(table, str(len(tables))))
